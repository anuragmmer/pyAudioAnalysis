...
# Calculate number of 15-second segments in the audio file
...
# Analyze each 15-second segment of the audio file
for i in range(n_segments):
    # Extract segment of audio signal
    y_segment = y[i * sr * 15:(i + 1) * sr * 15]
...
axs[0].plot(np.arange(n_segments) * 15, noise_values, 'ro-')
axs[0].set_ylabel('Noise (dB)')

axs[1].plot(np.arange(n_segments) * 15, tonal_shift_values, 'go-')
